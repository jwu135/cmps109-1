...
void window::select_object (size_t n) {
   if (n > window::objects.size()) 
      return;
   for (auto& obj: window::objects) 
      obj.is_selected = false;
   window::selected_obj = n;
   objects[n].is_selected = true;
}
...
void window::keyboard (GLubyte key, int x, int y) {
   ...
   switch (key) {
      case 'Q': case 'q': case ESC:
         ...
         break;
      case 'H': case 'h':
         move_selected_object (-window::moveby_width, 0);
         break;
      case 'J': case 'j':
         move_selected_object (0, -window::moveby_width);
         break;
      case 'K': case 'k':
         move_selected_object (0, window::moveby_width);
         break;
...
void window::special (int key, int x, int y) {
   ...
   switch (key) {
      case GLUT_KEY_LEFT: move_selected_object 
         (-window::moveby_width, 0); break;
      case GLUT_KEY_DOWN: move_selected_object 
         (0, -window::moveby_width); break;
